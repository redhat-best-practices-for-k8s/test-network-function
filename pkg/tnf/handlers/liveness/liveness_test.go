...
var (
	...
	livenessFilename      = "liveness.json"
	/* #nosec G101 */
	expectedPassPattern  = "(?m)liveness-defined"
	expectedFailPattern  = "(?m)liveness-not-defined"
...
func createTest() (*tnf.Tester, []reel.Handler, *gojsonschema.Result, error) {
	...
	return generic.NewGenericFromMap(livenessFilename, pathToTestSchemaFile, values)
}

func TestLiveness_Args(t *testing.T) {
...
func TestLiveness_GetIdentifier(t *testing.T) {
	test, handlers, jsonParseResult, err := createTest()
	...
	assert.Equal(t, identifier.LivenessURLIdentifier, (*test).GetIdentifier())
}

func TestLiveness_ReelFirst(t *testing.T) {
	...
	expectedCommand := fmt.Sprintf("oc get pod -n %s %s -o  go-template-file=./liveness.gotemplate",
...
func TestLiveness_ReelEof(t *testing.T) {
	_, handlers, jsonParseResult, err := createTest()
...
func TestLiveness_ReelTimeout(t *testing.T) {
	_, handlers, jsonParseResult, err := createTest()
...
func TestLiveness_ReelMatch(t *testing.T) {
	...
	assert.Equal(t, 1, len(handlers))
	handler := handlers[0]
	step := handler.ReelMatch(expectedFailPattern, "", "liveness-not-defined")

	assert.Nil(t, step)
	assert.Equal(t, tnf.FAILURE, (*tester).Result())

	step = handler.ReelMatch(expectedPassPattern, "", "liveness-defined")
