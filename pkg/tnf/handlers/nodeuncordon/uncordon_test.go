...
var (
	...
	uncordonFilename      = "uncordon.json"
	expectedPattern       = "(?m).*uncordoned"
	pathRelativeToRoot    = path.Join("..", "..", "..", "..")
	pathToTestSchemaFile  = path.Join(pathRelativeToRoot, genericTestSchemaFile)
	testNodeName          = "testNode123"
)

func createTest() (*tnf.Tester, []reel.Handler, *gojsonschema.Result, error) {
	values := make(map[string]interface{})
	values["NODE"] = testNodeName
	return generic.NewGenericFromMap(uncordonFilename, pathToTestSchemaFile, values)
}
...
func TestNodes_GetIdentifier(t *testing.T) {
	...
	assert.Equal(t, identifier.UncordonNodeURLIdentifier, (*test).GetIdentifier())
}
...
func TestNodes_ReelFirst(t *testing.T) {
	...
	assert.Equal(t, fmt.Sprintf("oc adm uncordon %s\n", testNodeName), step.Execute)
