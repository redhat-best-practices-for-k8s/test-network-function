...
func GetNodesList() (nodes map[string]configsections.Node) {
	...
	tester := nodenames.NewNodeNames(DefaultTimeout, map[string]*string{configsections.MasterLabel: nil}, false)
	...
	tester = nodenames.NewNodeNames(DefaultTimeout, map[string]*string{configsections.WorkerLabel: nil}, false)
	test, _ = tnf.NewTest(context.GetExpecter(), tester, []reel.Handler{tester}, context.GetErrorChannel())
	_, err = test.Run()
	if err != nil {
		log.Error("Unable to get node list ", ". Error: ", err)
	...
	context = interactive.GetContext()
	tester = nodenames.NewNodeNames(DefaultTimeout, nil, true)
	test, _ = tnf.NewTest(context.GetExpecter(), tester, []reel.Handler{tester}, context.GetErrorChannel())
	test.RunAndValidate()
	nodeNames = tester.GetNodeNames()
