...
2. Controls the actions that pods created by the operator can perform
(e.g. checks that the operator is not configured with privileged rights).
If clusterPermissions exists in the CSV,
and can perform the clusterPermissions[]. rules[].resourceNames is not empty then we fail the test.
