...
target host.
    ...
    ~/tnf$ go run tnf.redhat.com/tnf/cmd/ping/main.go -d success.log 10.5.0.3
...
to a timeout and killing the controlled subprocess.
    ...
    ~/tnf$ go run tnf.redhat.com/tnf/cmd/ping/main.go -d failure.log 10.3.0.99
    (timeout)
...
The final example shows a test-specific error in executing the test.
    ...
    ~/tnf$ go run tnf.redhat.com/tnf/cmd/ping/main.go -c 8 -t 10 10.5.0.99
...
In this example, two commands are supplied from a here document.
    ...
    ~/tnf$ env TERM=vt220 go run tnf.redhat.com/tnf/cmd/ssh/main.go -d ssh.log 'user@hhh:\S+\$ ' hhh -o 'PreferredAuthentications=publickey' <<EOF
    > echo foobar
    > date
    > EOF
...
a test configuration.
    ...
    ~/tnf$ env TERM=vt220 go run tnf.redhat.com/tnf/cmd/ssh/main.go -d ssh.log -T 'user@hhh:\S+\$ ' hhh -o 'PreferredAuthentications=publickey' <tests.json
