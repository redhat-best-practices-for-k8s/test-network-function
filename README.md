...
`TEST_CONFIGURATION_PATH` environment variable.
...
The [included example](test-network-function/tnf_config.yml) defines a single container to be tested,
...
[The included example config](test-network-function/tnf_config.yml) is set up with some examples of this:
...
A more extensive example of these sections is provided in [pkg/config/example.yaml](pkg/config/cnf_example.yaml)
